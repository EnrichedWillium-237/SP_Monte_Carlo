...
void RunCase( Int_t indx, Int_t nevents, Int_t evtmult, Bool_t v1odd, Double_t setv1, Double_t setv2, Bool_t eta_weights, Bool_t pt_weights, Bool_t conserve_pT, Bool_t addHoles, Bool_t flatten, Bool_t recenter, Int_t iseed, TString mtag, Int_t Ntries )
{
    ...
    for (int vbin = 0; vbin<nv1Ebins; vbin++) {
        ...
        event[vbin] = new MCEvent(0.0, setv2, 0.0, 0.0, 0.0, 0.0);
    }
    ...
    for (int nep = 0; nep<numEP; nep++) {
        ...
        hsub_phiPsi1[nep]   = new TH1D(Form("phiPsi1_ep%i_%i", nep,cindx), "", 150, -4,  4);
        hsub_phiPsi2[nep]   = new TH1D(Form("phiPsi2_ep%i_%i", nep,cindx), "", 150, -4,  4);
        hsub_Psi1[nep]      = new TH1D(Form("Psi1_ep%i_%i",    nep,cindx), "", 150, -4,  4);
        hsub_Psi2[nep]      = new TH1D(Form("Psi2_ep%i_%i",    nep,cindx), "", 150, -4,  4);
        hsub_Psi1lab[nep]   = new TH1D(Form("Psi1lab_ep%i_%i", nep,cindx), "", 150, -4,  4);
    ...
    Double_t spdenom2_2SE;
    Double_t spdenom2_3SE[2];
    ...
    double wmixnACsum[2];
    double wmixABCsum[2];
    double wmixAABBsum[2];
    double wmixAACCsum[2];
    double wmixBBCCsum[2];
    ...
    comp QmixnAA_pt[2][nptbins];
    comp QmixnAB_pt[2][nptbins];
    comp QmixnAC_pt[2][nptbins];

    comp Q1nAnorm_pt[2][nptbins];
    comp Q2nAnorm_pt[2][nptbins];
    comp QmixnAAnorm_pt[2][nptbins];
    comp QmixnABnorm_pt[2][nptbins];
    comp QmixnACnorm_pt[2][nptbins];

    double w1nAsum_pt[2][nptbins];
    double w2nAsum_pt[2][nptbins];
    double wmixnAAsum_pt[2][nptbins];
    double wmixnABsum_pt[2][nptbins];
    ...
    Double_t q2x_eta[2][netabins];
    Double_t q2y_eta[2][netabins];
    Double_t w1_eta[2][netabins];
    Double_t w2_eta[2][netabins];
    ...
    comp Q2nA_eta[2][netabins];
    comp QmixnAA_eta[2][netabins];
    comp QmixnAB_eta[2][netabins];
    comp QmixnAC_eta[2][netabins];

    comp Q1nAnorm_eta[2][netabins];
    comp Q2nAnorm_eta[2][netabins];
    comp QmixnAAnorm_eta[2][netabins];
    comp QmixnABnorm_eta[2][netabins];
    comp QmixnACnorm_eta[2][netabins];

    double w1nAsum_eta[2][netabins];
    double w2nAsum_eta[2][netabins];
    double wmixnAAsum_eta[2][netabins];
    double wmixnABsum_eta[2][netabins];
    ...
    for (int iside = 0; iside<2; iside++) {
        ...
        Q2ACnorm[iside] = zero;
        Q2BCnorm[iside] = zero;
        QmixnAAnorm[iside] = zero;
        QmixnABnorm[iside] = zero;
        QmixnACnorm[iside] = zero;
        QmixABCnorm[iside] = zero;
        QmixAABBnorm[iside] = zero;
        QmixAACCnorm[iside] = zero;
        QmixBBCCnorm[iside] = zero;
        ...
        for (int pbin = 0; pbin<nptbins; pbin++) {
            ...
            QmixnAA_pt[iside][pbin] = zero;
            QmixnAB_pt[iside][pbin] = zero;
            QmixnAC_pt[iside][pbin] = zero;

            Q1nAnorm_pt[iside][pbin] = zero;
            Q2nAnorm_pt[iside][pbin] = zero;
            QmixnAAnorm_pt[iside][pbin] = zero;
            QmixnABnorm_pt[iside][pbin] = zero;
            QmixnACnorm_pt[iside][pbin] = zero;

            w1nAsum_pt[iside][pbin] = 0;
            w2nAsum_pt[iside][pbin] = 0;
            wmixnAAsum_pt[iside][pbin] = 0;
            wmixnABsum_pt[iside][pbin] = 0;
        ...
        for (int ebin = 0; ebin<netabins; ebin++) {
            ...
            Q2nA_eta[iside][ebin] = zero;
            QmixnAA_eta[iside][ebin] = zero;
            QmixnAB_eta[iside][ebin] = zero;
            QmixnAC_eta[iside][ebin] = zero;

            Q1nAnorm_eta[iside][ebin] = zero;
            Q2nAnorm_eta[iside][ebin] = zero;
            QmixnAAnorm_eta[iside][ebin] = zero;
            QmixnABnorm_eta[iside][ebin] = zero;
            QmixnACnorm_eta[iside][ebin] = zero;

            w1nAsum_eta[iside][ebin] = 0;
            w2nAsum_eta[iside][ebin] = 0;
            wmixnAAsum_eta[iside][ebin] = 0;
            wmixnABsum_eta[iside][ebin] = 0;
...
//        Double_t PsiRP = ran->Uniform(-TMath::Pi(), TMath::Pi());
        ...
        for (int nep = 0; nep<numEP; nep++) {
            ...
            hsub_Psi2[nep]->Fill(bounds(2,Psi1[nep] - PsiRP));
        ...
        //-- calculate vn
        int ep1A, ep1B, ep1C, ep2A, ep2B, ep2C, epPOI;
        for (int iside = 0; iside<2; iside++) {
            // iside = 0: A = HFm (0), B = HFp (3), C = trackm (1), POI = trackp (2)
            if (iside == 0) {
                ep1A = 0;
                ep1B = 3;
                ep1C = 1;
            ...
            wmixnAAsum[iside] += w1[ep1A] * w2[ep2A] * (double)qcnt[epPOI];
            wmixnABsum[iside] += w1[ep1A] * w2[ep2B] * (double)qcnt[epPOI];
            wmixnACsum[iside] += w1[ep1A] * w2[ep2C] * (double)qcnt[epPOI];
            wmixABCsum[iside] += w1[ep1A] * w1[ep1B] * w2[ep2C];
            wmixAABBsum[iside] += w1[ep1A] * w2[ep2A] * w1[ep1B] * w2[ep2B];
            wmixAACCsum[iside] += w1[ep1A] * w2[ep2A] * w1[ep1C] * w2[ep2C];
            wmixBBCCsum[iside] += w1[ep1B] * w2[ep2B] * w1[ep1C] * w2[ep2C];
            ...
            comp mixnAA = Q1n * Q1A * std::conj(Q2A);
            comp mixnAB = Q1n * Q1A * std::conj(Q2B);
            comp mixnAC = Q1n * Q1A * std::conj(Q2C);
            comp mixABC = Q1A * Q1B * std::conj(Q2C);
            comp mixAABB = Q1A * Q2A * std::conj(Q1B) * std::conj(Q2B);
            comp mixAACC = Q1A * Q2A * std::conj(Q1C) * std::conj(Q2C);
            comp mixBBCC = Q1B * Q2B * std::conj(Q1C) * std::conj(Q2C);
            ...
            Q2AC[iside] += AC2;
            Q2BC[iside] += BC2;
            QmixnAA[iside] += mixnAA;
            QmixnAB[iside] += mixnAB;
            QmixnAC[iside] += mixnAC;
            QmixABC[iside] += mixABC;
            QmixAABB[iside] += mixAABB;
            QmixAACC[iside] += mixAACC;
            QmixBBCC[iside] += mixBBCC;
            ...
            comp mixnAAnorm = Q1n * Q1A/std::abs(Q1A) * (std::conj(Q2A)/std::abs(Q2A));
            comp mixnABnorm = Q1n * Q1A/std::abs(Q1A) * (std::conj(Q2B)/std::abs(Q2B));
            ...
            comp mixAABBnorm = (Q1A/std::abs(Q1A)) * (Q2A/std::abs(Q2A)) * (std::conj(Q1B)/std::abs(Q1B)) * (std::conj(Q2B)/std::abs(Q2B));
            comp mixAACCnorm = (Q1A/std::abs(Q1A)) * (Q2A/std::abs(Q2A)) * (std::conj(Q1C)/std::abs(Q1C)) * (std::conj(Q2C)/std::abs(Q2C));
            comp mixBBCCnorm = (Q1B/std::abs(Q1B)) * (Q2B/std::abs(Q2B)) * (std::conj(Q1C)/std::abs(Q1C)) * (std::conj(Q2C)/std::abs(Q2C));
            ...
            QmixnAAnorm[iside] += mixnAAnorm;
            QmixnABnorm[iside] += mixnABnorm;
            QmixnACnorm[iside] += mixnACnorm;
            QmixABCnorm[iside] += mixABCnorm;
            QmixAABBnorm[iside] += mixAABBnorm;
            QmixAACCnorm[iside] += mixAACCnorm;
            QmixBBCCnorm[iside] += mixBBCCnorm;
            ...
            for (int ebin = 0; ebin<netabins; ebin++) {
                ...
                q2x_eta[iside][ebin] = 0;
                q2y_eta[iside][ebin] = 0;
                w1_eta[iside][ebin] = 0;
                w2_eta[iside][ebin] = 0;
            ...
            for (int j = 0; j<evtmult; j++) {
                ...
                for (int ebin = 0; ebin<netabins; ebin++) {
                    if (eta>=etabins[ebin] && eta<etabins[ebin+1] && pt>0.3 && pt<3.0) {
                        ...
                        q2x_eta[iside][ebin] += ww2*TMath::Cos(2*ph);
                        q2y_eta[iside][ebin] += ww2*TMath::Sin(2*ph);
                        w1_eta[iside][ebin] += ww1;
                        w2_eta[iside][ebin] += ww2;
                        qcnt_eta[iside][ebin]++;
                    }
                }
            }
            for (int pbin = 0; pbin<nptbins; pbin++) {
                ...
                wmixnAAsum_pt[iside][pbin] += w1[ep1A] * w2[ep2A] * (double)qcnt_pt[iside][pbin];
                wmixnABsum_pt[iside][pbin] += w1[ep1A] * w2[ep2B] * (double)qcnt_pt[iside][pbin];
                ...
                comp mixnAA_pt = Q1n_pt * Q1A * std::conj(Q2A);
                comp mixnAB_pt = Q1n_pt * Q1A * std::conj(Q2B);
                comp mixnAC_pt = Q1n_pt * Q1A * std::conj(Q2C);

                Q1nA_pt[iside][pbin] += nA1_pt;
                Q2nA_pt[iside][pbin] += nA2_pt;
                QmixnAA_pt[iside][pbin] += mixnAA_pt;
                QmixnAB_pt[iside][pbin] += mixnAB_pt;
                ...
                comp mixnAAnorm_pt = Q1n_pt * Q1A/std::abs(Q1A) * (std::conj(Q2A)/std::abs(Q2A));
                comp mixnABnorm_pt = Q1n_pt * Q1A/std::abs(Q1A) * (std::conj(Q2B)/std::abs(Q2B));
                ...
                QmixnAAnorm_pt[iside][pbin] += mixnAAnorm_pt;
                QmixnABnorm_pt[iside][pbin] += mixnABnorm_pt;
            ...
            for (int ebin = 0; ebin<netabins; ebin++) {
                ...
                w2nAsum_eta[iside][ebin] += w2[ep2A] * (double)qcnt_eta[iside][ebin];
                wmixnAAsum_eta[iside][ebin] += w1[ep1A] * w2[ep2A] * (double)qcnt_eta[iside][ebin];
                wmixnABsum_eta[iside][ebin] += w1[ep1A] * w2[ep2B] * (double)qcnt_eta[iside][ebin];
                wmixnACsum_eta[iside][ebin] += w1[ep1A] * w2[ep2C] * (double)qcnt_eta[iside][ebin];

                comp Q1n_eta(q1x_eta[iside][ebin], q1y_eta[iside][ebin]);
                comp Q2n_eta(q2x_eta[iside][ebin], q2y_eta[iside][ebin]);

                comp nA1_eta = Q1n_eta * std::conj(Q1A);
                comp nA2_eta = Q2n_eta * std::conj(Q2A);
                comp mixnAA_eta = Q1n_eta * Q1A * std::conj(Q2A);
                comp mixnAB_eta = Q1n_eta * Q1A * std::conj(Q2B);
                comp mixnAC_eta = Q1n_eta * Q1A * std::conj(Q2C);

                Q1nA_eta[iside][ebin] += nA1_eta;
                Q2nA_eta[iside][ebin] += nA2_eta;
                QmixnAA_eta[iside][ebin] += mixnAA_eta;
                QmixnAB_eta[iside][ebin] += mixnAB_eta;
                QmixnAC_eta[iside][ebin] += mixnAC_eta;

                comp nA1norm_eta = Q1n_eta * (std::conj(Q1A)/std::abs(Q1A));
                comp nA2norm_eta = Q2n_eta * (std::conj(Q2A)/std::abs(Q2A));
                comp mixnAAnorm_eta = Q1n_eta * Q1A/std::abs(Q1A) * (std::conj(Q2A)/std::abs(Q2A));
                comp mixnABnorm_eta = Q1n_eta * Q1A/std::abs(Q1A) * (std::conj(Q2B)/std::abs(Q2B));
                comp mixnACnorm_eta = Q1n_eta * Q1A/std::abs(Q1A) * (std::conj(Q2C)/std::abs(Q2C));

                Q1nAnorm_eta[iside][ebin] += nA1norm_eta;
                Q2nAnorm_eta[iside][ebin] += nA2norm_eta;
                QmixnAAnorm_eta[iside][ebin] += mixnAAnorm_eta;
                QmixnABnorm_eta[iside][ebin] += mixnABnorm_eta;
    ...
    rescor2_2 = sqrt( fabs(Q2ABnorm[0].real())/(double)nevts );
    rescor2_2SE->Fill(rescor2_2);

    spdenom1_2SE = sqrt( fabs(Q1AB[0].real()/w1ABsum[0]) );
    SPdenom1_2SE->Fill(spdenom1_2SE);

    spdenom2_2SE = sqrt( fabs(Q2AB[0].real()/w2ABsum[0]) );
    SPdenom2_2SE->Fill(spdenom2_2SE);
    ...
    for (int iside = 0; iside<2; iside++) {
        ...
        double naaep = QmixnAAnorm[iside].real()/nEP1count;
        double nabep = QmixnABnorm[iside].real()/nEP1count;
        double nacep = QmixnACnorm[iside].real()/nEP1count;
        double abcep = QmixABCnorm[iside].real()/nEP1evts;
        double aabbep = QmixAABBnorm[iside].real()/nEP1evts;
        double aaccep = QmixAACCnorm[iside].real()/nEP1evts;
        double bbccep = QmixBBCCnorm[iside].real()/nEP1evts;
        ...
        double naasp = QmixnAA[iside].real()/wmixnAAsum[iside];
        double nabsp = QmixnAB[iside].real()/wmixnABsum[iside];
        double nacsp = QmixnAC[iside].real()/wmixnACsum[iside];
        double abcsp = QmixABC[iside].real()/wmixABCsum[iside];
        double aabbsp = QmixAABB[iside].real()/wmixAABBsum[iside];
        double aaccsp = QmixAACC[iside].real()/wmixAACCsum[iside];
        double bbccsp = QmixBBCC[iside].real()/wmixBBCCsum[iside];
        ...
        hQmixAABB_final[iside]->Fill(aabbsp);
        hQmixAACC_final[iside]->Fill(aaccsp);
        hQmixBBCC_final[iside]->Fill(bbccsp);
        ...
        hQmixAABBnorm_final[iside]->Fill(aabbep);
        hQmixAACCnorm_final[iside]->Fill(aaccep);
        hQmixBBCCnorm_final[iside]->Fill(bbccep);


        rescor1_3[iside] = sqrt( fabs(ab1ep*ac1ep/bc1ep) );
        rescor1_3SE[iside]->Fill(rescor1_3[iside]);

        rescor2_3[iside] = sqrt( fabs(ab2ep*ac2ep/bc2ep) );
        rescor2_3SE[iside]->Fill(rescor2_3[iside]);

        rescorMix[iside] = sqrt( fabs(abcep * rescor2_3[iside]) );
        ...
        epv2_2SE[iside] = na2ep/rescor2_2;
        EPv2_2SE[iside]->Fill(epv2_2SE[iside]);

        epv2_3SE[iside] = na2ep/rescor2_3[iside];
        ...
        spdenom2_3SE[iside] = sqrt( fabs(ab2sp*ac2sp/bc2sp) );
        SPdenom2_3SE[iside]->Fill(spdenom2_3SE[iside]);

        spdenom_mix[iside] = sqrt( fabs(abcsp) * spdenom2_3SE[iside] );
        ...
        spv2_2SE[iside] = na2sp/spdenom2_2SE;
        SPv2_2SE[iside]->Fill(spv2_2SE[iside]);

        spv2_3SE[iside] = na2sp/spdenom2_3SE[iside];
        ...
        for (int pbin = 0; pbin<nptbins; pbin++) {
            ...
            double naaep_pt = QmixnAAnorm_pt[iside][pbin].real()/nEP1count_pt;
            double nabep_pt = QmixnABnorm_pt[iside][pbin].real()/nEP1count_pt;
            ...
            double naasp_pt = QmixnAA_pt[iside][pbin].real()/wmixnAAsum_pt[iside][pbin];
            double nabsp_pt = QmixnAB_pt[iside][pbin].real()/wmixnABsum_pt[iside][pbin];
            ...
            epv2_2SE_pt[iside][pbin] = na2ep_pt/rescor2_2;
            EPv2_2SE_pt[iside][pbin]->Fill(epv2_2SE_pt[iside][pbin]);

            epv2_3SE_pt[iside][pbin] = na2ep_pt/rescor2_3[iside];
            ...
            spv2_2SE_pt[iside][pbin] = na2sp_pt/spdenom2_2SE;
            SPv2_2SE_pt[iside][pbin]->Fill(spv2_2SE_pt[iside][pbin]);

            spv2_3SE_pt[iside][pbin] = na2sp_pt/spdenom2_3SE[iside];
        ...
        for (int ebin = 0; ebin<netabins; ebin++) {
            ...
            double naaep_eta = QmixnAAnorm_eta[iside][ebin].real()/nEP1count_eta;
            double nabep_eta = QmixnABnorm_eta[iside][ebin].real()/nEP1count_eta;
            double nacep_eta = QmixnACnorm_eta[iside][ebin].real()/nEP1count_eta;
            double na2ep_eta = Q2nAnorm_eta[iside][ebin].real()/(double)ncount_eta[iside][ebin];

            double na1sp_eta = Q1nA_eta[iside][ebin].real()/w1nAsum_eta[iside][ebin];
            double naasp_eta = QmixnAA_eta[iside][ebin].real()/wmixnAAsum_eta[iside][ebin];
            double nabsp_eta = QmixnAB_eta[iside][ebin].real()/wmixnABsum_eta[iside][ebin];
            double nacsp_eta = QmixnAC_eta[iside][ebin].real()/wmixnACsum_eta[iside][ebin];
            double na2sp_eta = Q2nA_eta[iside][ebin].real()/w2nAsum_eta[iside][ebin];

            epv1obs_eta[iside][ebin] = na1ep_eta;
            EPv1obs_eta[iside][ebin]->Fill(epv1obs_eta[iside][ebin]);

            epv2obs_eta[iside][ebin] = na2ep_eta;
            EPv2obs_eta[iside][ebin]->Fill(epv2obs_eta[iside][ebin]);
            ...
            epv2_2SE_eta[iside][ebin] = na2ep_eta/rescor2_2;
            EPv2_2SE_eta[iside][ebin]->Fill(epv2_2SE_eta[iside][ebin]);

            epv2_3SE_eta[iside][ebin] = na2ep_eta/rescor2_3[iside];
            EPv2_3SE_eta[iside][ebin]->Fill(epv2_3SE_eta[iside][ebin]);
            ...
            spv1num_mix_eta[iside][ebin] = naasp_eta;
            SPv1numMix_eta[iside][ebin]->Fill(spv1num_mix_eta[iside][ebin]);

            spv2num_eta[iside][ebin] = na2sp_eta;
            SPv2num_eta[iside][ebin]->Fill(na2sp_eta);
            ...
            spv2_2SE_eta[iside][ebin] = na2sp_eta/spdenom2_2SE;
            SPv2_2SE_eta[iside][ebin]->Fill(spv2_2SE_eta[iside][ebin]);

            spv2_3SE_eta[iside][ebin] = na2sp_eta/spdenom2_3SE[iside];
            SPv2_3SE_eta[iside][ebin]->Fill(spv2_3SE_eta[iside][ebin]);

            hncnt_eta[iside][ebin]->Fill(ncount_eta[iside][ebin]);

            // Q-vector averages
            hQ1nA[iside][ebin]->Fill(na1sp_eta);
            hQ2nA[iside][ebin]->Fill(na2sp_eta);
            hQmixnAA[iside][ebin]->Fill(naasp_eta);
            hQmixnAB[iside][ebin]->Fill(nabsp_eta);
            hQmixnAC[iside][ebin]->Fill(nacsp_eta);

            // normalized Q-vector averages
            hQ1nAnorm[iside][ebin]->Fill(na1ep_eta);
            hQ2nAnorm[iside][ebin]->Fill(na2ep_eta);
            hQmixnAAnorm[iside][ebin]->Fill(naaep_eta);
            hQmixnABnorm[iside][ebin]->Fill(nabep_eta);
    ...
    TString tag = Form("data_%s",mtag.Data());
    ...
    fout<<"rescor2_2:          "<<Form("%.6f",rescor2_2)      <<endl;
    fout<<"rescor2_3    (HFm): "<<Form("%.6f",rescor2_3[0])   <<"\t (HFp): "<<Form("%.6f",rescor2_3[1])<<endl;
    ...
    fout<<"spdenom2_2SE:       "<<Form("%.6f",spdenom2_2SE)   <<endl;
    fout<<"spdenom2_3SE (HFm): "<<Form("%.6f",spdenom2_3SE[0])<<"\t (HFp): "<<Form("%.6f",spdenom2_3SE[1])<<endl;
    ...
    cout<<"rescor2_2:          "<<Form("%.6f",rescor2_2)      <<endl;
    cout<<"rescor2_3    (HFm): "<<Form("%.6f",rescor2_3[0])   <<"\t (HFp): "<<Form("%.6f",rescor2_3[1])<<endl;
    ...
    cout<<"spdenom2_2SE:       "<<Form("%.6f",spdenom2_2SE)   <<endl;
    cout<<"spdenom2_3SE (HFm): "<<Form("%.6f",spdenom2_3SE[0])<<"\t (HFp): "<<Form("%.6f",spdenom2_3SE[1])<<endl;
